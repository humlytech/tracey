...
Tracey scans comments in: Rust, Swift, TypeScript, TSX, JavaScript, JSX, Go, C, C++, Objective-C, Objective-C++, Java, Kotlin, Scala, Groovy, C#, Zig, PHP, YAML (`.yml`, `.yaml`), JSON5 (`.json5`), JSONC (`.jsonc`), Terraform (`.tf`, `.tfvars`), Docker (`Dockerfile`, `Dockerfile.*`, `.dockerignore`).
