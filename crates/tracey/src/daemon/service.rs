...
#[derive(Debug, Clone)]
pub(crate) struct HistoricalRuleText {
...
impl TraceyService {
    ...
    fn resolve_spec_impl(
        ...
        crate::bridge::query_exec::resolve_spec_impl(spec, impl_name, config)
    }
}
...
impl TraceyDaemon for TraceyService {
    ...
    async fn rule(&self, rule_id: RuleId) -> Option<RuleInfo> {
        ...
        crate::bridge::query_exec::rule(&data, project_root, rule_id).await
    }

    /// Get current configuration
    async fn config(&self) -> ApiConfig {
        let data = self.inner.engine.data().await;
        crate::bridge::query_exec::config(&data)
    }
    ...
    async fn validate(&self, req: ValidateRequest) -> ValidationResult {
        let data = self.inner.engine.data().await;
        crate::bridge::query_exec::validate(&data, req)
    }
...
pub(crate) async fn load_previous_rule_text_from_git(
    project_root: &Path,
    source_file: &str,
